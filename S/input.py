...
class Input:
    ...
    project = "S"  # a directory of this name is created, containing this input.py

    # Habitat:
    population = 100  # [people], can be a number or list of numbers
    ...
    cylinderLengthToRotRadius = 0.5  # for Cylinder
    tubeRadiusToRotRadius = 0.1  # for Tube and DumbbellTube
    oblateMinorToRotRadius = .25  # for Oblate
    torusHabToRotRadius = 0.1  # for Torus
    dumbbellMinorToRotRadius = 0.2  # for Dumbbell and DumbbellTube
    ...
    # multiple runs:
    numberRuns = 7  # set e.g. to 6 to compute examples with changeParameters
    iRun = 0  # initialized
    label = []  # initialized

    def changeParameters(self, iRun):
        self.iRun = iRun
        self.maxGravity = 9.81
        if iRun == 0:  # Cylinder
            self.maxGravity /= 2
            self.label.append("Cylinder")
        if iRun == 1:
